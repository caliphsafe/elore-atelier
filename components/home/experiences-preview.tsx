      ...
      <div className="editorial-container">
        <div className="grid gap-7 lg:grid-cols-[1.15fr_0.85fr] lg:items-stretch">
          ...
          <div className="flex flex-col rounded-luxe border thin-border bg-white p-6 shadow-soft md:p-8 lg:p-9">
            ...
            <div className="mt-9 grid gap-5">{offerings.map((offering) => <div key={offering.title} className="border-b thin-border pb-5 last:border-none last:pb-0"><h3 className="serif-display text-3xl leading-tight text-brand-olive">{offering.title}</h3><p className="mt-3 text-sm leading-7 text-brand-olive/75 md:text-base">{offering.text}</p></div>)}</div>
            <div className="mt-9 flex flex-col gap-3 sm:flex-row"><Link href="/experiences" className="inline-flex justify-center rounded-full bg-brand-olive px-6 py-3 text-xs uppercase tracking-[0.24em] text-white transition hover:bg-brand-maroon">Explore Experiences</Link><Link href="/contact" className="inline-flex justify-center rounded-full border border-brand-olive px-6 py-3 text-xs uppercase tracking-[0.24em] text-brand-olive transition hover:border-brand-maroon hover:text-brand-maroon">Inquire to Book</Link></div>
          </div>
        </div>
      </div>
    </section>
