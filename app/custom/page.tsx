    ...
    <main className="custom-page">
      ...
      <section id="custom-process" className="custom-process relative overflow-hidden">
        <Image
          ...
          className="pointer-events-none absolute inset-y-0 left-0 z-0 object-contain object-left opacity-30 mix-blend-soft-light"
          aria-hidden="true"
        />
        ...
        <div className="editorial-container custom-process__inner relative z-10">
          ...
          <div className="custom-process__list">
            {stages.map((stage, index) => (
              <div className="custom-process__item" key={stage}>
          ...
          <div className="custom-fit__steps bg-brand-sand/72 backdrop-blur-[2px]">
            {measureSteps.map(([number, title, description]) => (
              <div className="custom-fit__step" key={number}>
                ...
                <div>
                  <h3 className="serif-display">{title}</h3>
                  <p>{description}</p>
                </div>
              </div>
            ))}
            <div className="custom-fit__help">
              <h3 className="serif-display">Need Assistance?</h3>
              <p>If you have any questions about sizing or would like guidance before placing your custom order, we're here to help. Contact us at: <a href="mailto:Salome@EloreAtelier.com">Salome@EloreAtelier.com</a></p>
            </div>
          </div>
        </div>
      </section>

      <section className="custom-close">
        <div className="editorial-container custom-close__inner">
          <h2 className="serif-display">Begin your custom consultation.</h2>
          <Link href="/contact" className="btn-light">Submit Inquiry</Link>
        </div>
      </section>
    </main>
  );
